...
export default function WithdrawSent() {
  ...
  const load = useCallback(async (p, s) => {
    ...
    setError('');
    try {
      const res = await fetchWithdrawals({ status: 'approved', page: p, search: s });
  ...
  return (
    <div className="deposit-update-page">
      <header className="deposit-header">
        <div>
          <h1>Withdraw Sent</h1>
          <p className="deposit-subheader">Approved & sent withdrawals — {total} total</p>
        </div>
      </header>
      ...
      <div className="deposit-controls">
        <span className="deposit-entries-label">{total} sent</span>
        ...
        <table className="deposit-table">
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan="8" className="deposit-empty">No sent withdrawals</td></tr>
            ) : (
              withdrawals.map((row, i) => (
                <tr key={row._id}>
                  ...
                  <td className="deposit-mono">{row.paymentRef || '-'}</td>
